...
Proof:
If Di = {n>=0, pii^n > 0}
Take n1, n2 in D1 st n2 - n1 = 1

for n in N, n = qn1 + r, r <= n-1
    n = (q-r)n1 + rn2 .

For large n, q-r > 0, and (q-r)n1 and rn2 are both positive and in D1.

> Exercise: An irreducable chain is aperiodic iff $\exists n st p_{ij}^{(n)} > 0 \forall i,j \in S$

### Theorem

Let {X_n} be irreducable of period d > 1. Then it can be decomposed to a disjoint union of sets C0, C1, ... Cd-1 such that

sum_{j in C_r+1} p_ij = 1  forall i in C_r forall r

Pf: 

Define a relation i <-d-> j iff p_ij^(nd) > 0 for some n in N in an irreducable chain of period d.

This is transitive and reflexive.

Symmetric - 

p_ij^(alpha d) > 0 for some alpha

Since the chain is irr, j --> i, 

So exists beta > 0 st p_ji^(beta) > 0

=> p_ii^(alpha d + b) >= pij^(alpha d) pji^(beta) > 0

but period of i is d, which means d | alpha d + beta, => d | beta

S can be written as a disjoint union of the equivalence classes.

Pick i0 in S, and denote its equivalence class (under <--d-->) as C0.
Then pick i_1 in S st p_i0i1 > 0. Denote its equivalence class as C1.
Similarly do until i_d-1 and C_d-1.

Note that id MUST be in C0, because there exists a path of length d.

Let i be in C0, and pij > 0 for some j in S, then j must be in C1.

Suppose j not in C1, but in C2.

Then consider PATH (i0 -> i -> j -> i2 -> i3 -> ... -> i_d-1)

This i0 -> i is of length ad, and j -> i2  is of length bd. Then, the length of this new PATH is d+1, which contradicts the fact that i_d must be in C0.
